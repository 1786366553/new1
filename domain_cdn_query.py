...
def cdn_1():
    ...
    collection=db.domain_cdn_query
    for data in collection.find({'flag':0,'id_count':{'$gt':0,'$lt':250000}}):
        ...
        ns_record = domain_online_query(domain_2,"NS")['query_answer']
        dns_record = [{'A_record':A_record_handle(a_record),'CNAME_record':CNAME_record_handle(cname_record),'NS_record':NS_record_handle(ns_record),'insert_time':datetime.now().strftime('%Y-%m-%d %H:%M:%S')}]
        collection.update({'_id':data['_id']},{'$set':{'DNS_record':dns_record,'flag':1}})
